...
with open('stats.csv', 'w') as csvfile:
    ...
    for row in table.find_all('tr')[1:270]:
        ...
        writer.writerow({
            ...
            'field_goal_attempted': BeautifulSoup(str(col[6]), 'lxml').text,
            'field_goal_percentage': BeautifulSoup(str(col[7]), 'lxml').text,
            'three_point_made': BeautifulSoup(str(col[8]), 'lxml').text,
            'three_point_attempted': BeautifulSoup(str(col[9]), 'lxml').text,
            'three_point_percentage': BeautifulSoup(str(col[10]), 'lxml').text,
